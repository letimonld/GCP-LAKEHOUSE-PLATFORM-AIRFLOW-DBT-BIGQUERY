...
*   **ELT Pipeline**:
    ...
    *   Airflow job scheduler triggers DBT jobs for BigQuery to clean and load data into the **Silver Intermediate** layer using SCD Type 2 to maintain historical data.
    *   Airflow then triggers another DBT job to load the latest transformed data into the **Gold Curated** layer.
...
2.  **Restore the Database**: Restore the `.bak` file to your SQL Server instance using SQL Server Management Studio.
